...
class CommonModel extends Model
{
    ...
    public function update_batchdata($table, $postdata = array(), $conditions = '')
    {
        $builder = $this->db->table($table);
        $builder->where($conditions);
        $builder->update($postdata);
    ...
    public function monthShortName($monthNo)
    {
        if ($monthNo == 1) {
            $month_name = 'Jan';
        }
    ...
    public function getParentChildTableField($typeId)
    {
        if ($typeId == 1) { //Brand
    ...
    public function checkSellerBuyer($uid)
    {
    ...
    /* check document upload */
    public function pagination($page = 0, $perPage = 0, $total = 0)
    {
    ...
    /**
         * Find records using WHERE IN clause
         *
         * @param string $table Table name
         * @param string $column Column name for WHERE IN
         * @param array $values Array of values for WHERE IN clause
         * @param string $return_type Return type (array, row, row-array, result-array, count)
         * @param string $select Fields to select
         * @return mixed
         */
    public function find_where_in($table, $column, $values, $return_type = 'array', $select = '*')
    {
        $result = array();
        $builder = $this->db->table($table);
        $builder->select($select);
    ...
    // Example 1: Basic WHERE IN query
    /*$values = [1, 2, 3];
    $result = $this->CommonModel->find_where_in('table_name', 'column_name', $values);

    // Example 2: With specific return type and select fields
    $values = [1, 2, 3];
    $result = $this->CommonModel->find_where_in(
        'table_name',                          // table name
        'column_name',                         // column for WHERE IN
        $values,                               // array of values
        'result-array',                        // return type (as array)
        'id, name, email'                      // specific fields to select
    );

    // Example 3: Count results
    $values = [1, 2, 3];
    $count = $this->CommonModel->find_where_in(
        'table_name',
        'column_name',
        $values,

    ...
    <section style="padding: 80px 0; height: 80vh; margin: 0 15px;">
        <div style="max-width: 600px; background: #ffffff; margin: 0 auto; border-radius: 15px; padding: 20px 15px; box-shadow: 0 0 30px -5px #ccc;">
          ...
          <div>
            ...
            <?php if($status == 1){?>
              <h4 style="text-align: center; font-size: 15px; color: green; font-family: sans-serif;">You Have Successfully Accepted Enquiry Invitation For Quotation Submission Request</h4>
            <?php } else {?>
              <h4 style="text-align: center; font-size: 15px; color: red; font-family: sans-serif;">You Have Successfully Rejected Enquiry Invitation For Quotation Submission Request</h4>
            <?php }?>
                ...
                <tr>
                  <td colspan="2" style="padding: 10px; background: #89b33c; text-align: left; color: #fff;font-family: sans-serif;font-size: 15px; font-weight: 600; text-align:center">
                    <table cellpadding="5" cellspacing="3" style="width: 100%;  border-spacing: 2px; border-collapse:collapse;">
                      ...
                      <?php
                      $enqItems = $this->common_model->find_data('ecomm_enquiry_products', 'array', ['enq_id' => $enq_id]);
                      if($enqItems){ $sl=1; foreach($enqItems as $enqItem){
                        $getItem = $this->common_model->find_data('ecomm_company_items', 'row', ['id' => $enqItem->product_id]);

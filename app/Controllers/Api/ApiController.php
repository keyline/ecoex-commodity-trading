...
class ApiController extends BaseController
{
        ...
        if ($headerData['Key'] == 'Key: ' . getenv('app.PROJECTKEY')) {
            ...
            if ($getSubEnquiryItems) {
                foreach ($getSubEnquiryItems as $getSubEnquiryItem) {
                    ...
                    $total_win_price_array[] = [
                        ...
                        'comparison_percentage'         => number_format(abs((($before_price > 0) ? (($difference_price / $before_price) * 100) : ($difference_price))),2),
                    ];
                }
            }
            ...
            $sql = "WITH top_vendors AS (
                            ...
                            FROM 
                                ecomm_sub_enquires
                            WHERE 
                                assigned_date BETWEEN '$startDate' AND '$endDate'
                            GROUP BY 
                                vendor_id
                            ORDER BY 
                                total_qty DESC
            ...
            $apiResponse = [
                'top_prices'        => $final,
                'top_qty'           => $top_qty,
                'top_buyers'        => $top_buyers_final_final,
            ];
                ...
                if ($checkPhone <= 0) {
                    if ($requestData['password'] == $requestData['confirm_password']) {
                        ...
                        $postData       = [
                            ...
                            'contact_person_name'       => $requestData['cp_name'],
    ...
    public function signin()
    {
        ...
        if ($headerData['Key'] == 'Key: ' . getenv('app.PROJECTKEY')) {
            ...
            $orderBy[0]                 = ['field' => 'id', 'type' => 'DESC'];
            $checkUser                  = $this->common_model->find_data('ecomm_users', 'row', ['email' => $email, 'type' => $type], '', '', '', $orderBy);
            // pr($checkUser);
            if ($checkUser) {
    ...
    protected function completeRequestCount($uId)
    {
        $sql = "SELECT COUNT(DISTINCT ecomm_enquires.id) AS total_count
        FROM ecomm_enquires
        INNER JOIN ecomm_enquiry_vendor_shares ON ecomm_enquiry_vendor_shares.enq_id = ecomm_enquires.id
        INNER JOIN ecomm_sub_enquires ON ecomm_sub_enquires.enq_id = ecomm_enquires.id
        INNER JOIN ecomm_enquiry_products ON ecomm_enquiry_products.enq_id = ecomm_enquires.id
        WHERE ecomm_sub_enquires.plant_id = ?
          AND ecomm_sub_enquires.status = 12.12
          AND ecomm_enquires.status = 12
          AND ecomm_enquiry_products.status != 3";


        $builder = $this->db->query($sql, [$uId]);
                ...
                if ($getUser) {
                    ...
                    if ($rows) {
                        ...
                        $apiResponse = [
                            ...
                            'vendor_name'                       => (($getVendor) ? $getVendor->company_name : ''),
                            'tentative_collection_date'         => (($getEnquiry) ? date_format(date_create($getEnquiry->tentative_collection_date), "M d, Y h:i A") : ''),
                            ...
                            'pickup_schedule_remarks'           => $rows[0]->pickup_schedule_remarks,
    ...
    protected function getEnquiryCount($uId, $status = 0, $excludeEnquireStatus = 14)
    {
        $builder = $this->db->table('ecomm_enquires');
        $builder->join(
            'ecomm_enquiry_vendor_shares',
            'ecomm_enquiry_vendor_shares.enq_id = ecomm_enquires.id',
            'INNER'
        );
        $builder->where('ecomm_enquiry_vendor_shares.vendor_id', $uId);
        $builder->where('ecomm_enquiry_vendor_shares.status', $status);
        $builder->where('ecomm_enquires.status !=', $excludeEnquireStatus);
        ...
        if ($headerData['Key'] == 'Key: ' . getenv('app.PROJECTKEY')) {
            ...
            if ($getTokenValue['status']) {
                ...
                if ($getUser) {
                    ...
                    //__________________ new request count _____________________

                    // $new_count              = $this->common_model->find_data('ecomm_enquiry_vendor_shares', 'count', ['vendor_id' => $uId, 'status' => 0]);
                    # new code added on :10/04/2025
                    $new_count = $this->getEnquiryCount($uId, 0);  # 0 = new 
                    // _________________ new request count _____________________
                    ...
                    $groupBy[0]             = 'sub_enquiry_no';
                    $process_request        = $this->common_model->find_data('ecomm_sub_enquires', 'count', ['vendor_id' => $uId, 'status<=' => 10.10, 'assigned_date!=' => ''], '', '', $groupBy);

                    //__________________ new rejected count _____________________
                    // $rejected_enquiry       = $this->common_model->find_data('ecomm_enquiry_vendor_shares', 'count', ['vendor_id' => $uId, 'status' => 3]);
                    # new code added on :10/04/2025
                    $rejected_enquiry       = $this->getEnquiryCount($uId, 3);  # 3 = rejected ;
                    //__________________ new rejected count _____________________
        ...
        if ($headerData['Key'] == 'Key: ' . getenv('app.PROJECTKEY')) {
            ...
            if ($getTokenValue['status']) {
                ...
                if ($getUser) {
                    ...
                    $select             = 'ecomm_enquires.*';
                    $join[0]            = ['table' => 'ecomm_enquiry_vendor_shares', 'field' => 'enq_id', 'table_master' => 'ecomm_enquires', 'field_table_master' => 'id', 'type' => 'INNER'];
                    $rows               = $this->common_model->find_data('ecomm_enquires', 'array', ['ecomm_enquiry_vendor_shares.vendor_id' => $uId, 'ecomm_enquiry_vendor_shares.status' => 1, 'ecomm_enquires.status<=' => 2], $select, $join, '', $orderBy, $limit, $offset);
                    // $this->db = \Config\Database::connect();
                    // echo $this->db->getLastQuery();die;
                    if ($rows) {
                        foreach ($rows as $row) {
        ...
        if ($headerData['Key'] == 'Key: ' . getenv('app.PROJECTKEY')) {
            ...
            if ($getTokenValue['status']) {
                ...
                if ($getUser) {
                    ...
                    $select             = 'ecomm_enquires.*';
                    $join[0]            = ['table' => 'ecomm_sub_enquires', 'field' => 'enq_id', 'table_master' => 'ecomm_enquires', 'field_table_master' => 'id', 'type' => 'INNER'];
                    // $rows               = $this->common_model->find_data('ecomm_enquires', 'array', ['ecomm_sub_enquires.vendor_id' => $uId, 'ecomm_sub_enquires.status' => 2, 'ecomm_enquires.status!=' => 14], $select, $join, '', $orderBy, $limit, $offset);
                    $rows               = $this->common_model->find_data('ecomm_enquires', 'array', ['ecomm_sub_enquires.vendor_id' => $uId, 'ecomm_sub_enquires.status' => 12.12, 'ecomm_enquires.status' => 12], $select, $join, '', $orderBy, $limit, $offset);
                    // $this->db = \Config\Database::connect();
                    // echo $this->db->getLastQuery();die;
                    if ($rows) {
                        foreach ($rows as $row) {
    ...
    */
    ...
    /* new vendor app flow update in february 2026 */
        # vendor price list
        ...
        # get vendor enquiry items
        public function getVendorEnquiryItems()
        {
            $apiStatus          = TRUE;
            $apiMessage         = '';
            $apiResponse        = [];
        ...
        # vendor enquiry request accet, reject, quotation submission, auto final assign
        public function vendorPendingRequestAcceptRejectNew()
        {
            $apiStatus          = TRUE;
            $apiMessage         = '';
            $apiResponse        = [];
            ...
            if ($headerData['Key'] == 'Key: ' . getenv('app.PROJECTKEY')) {
                ...
                if ($getTokenValue['status']) {
                    ...
                    $getUser    = $this->common_model->find_data('ecomm_users', 'row', ['id' => $uId]);
                    if ($getUser) {
                        $checkEnquiry = $this->common_model->find_data('ecomm_enquires', 'row', ['id' => $enq_id]);
                        if($checkEnquiry){

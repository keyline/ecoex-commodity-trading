                ...
                if($headerData['Key'] == 'Key: '.getenv('app.PROJECTKEY')){
                    ...
                    if($getTokenValue['status']){
                        ...
                        if($getUser){
                            ...
                            $rows               = $this->common_model->find_data('ecomm_enquires', 'array', ['ecomm_enquiry_vendor_shares.vendor_id' => $uId, 'ecomm_enquiry_vendor_shares.status' => 2, 'ecomm_enquires.status' => 12], $select, $join, '', $orderBy, $limit, $offset);
                            // $this->db = \Config\Database::connect();
                            // echo $this->db->getLastQuery();die;
                            if($rows){
                                foreach($rows as $row){

...
class MemberTypeController extends BaseController {
	...
	public function __construct()
    {
        ...
        $this->data = array(
            ...
            'session'               => $session,
            'title'                 => 'Vendor Type',
            'controller_route'      => 'member-types',
            'controller'            => 'MemberTypeController',
            'table_name'            => 'ecomm_member_types',
            'primary_key'           => 'id'
        );
    }
    public function list()
    {
        if(!$this->common_model->checkModuleFunctionAccess(12,55)){
    ...
    public function add()
    {
        if(!$this->common_model->checkModuleFunctionAccess(12,57)){
            $data['action']             = 'Access Forbidden';
            $title                      = $data['action'].' '.$this->data['title'];
            $page_name                  = 'access-forbidden';        
            echo $this->layout_after_login($title,$page_name,$data);
        ...
        $data['action']             = 'Add';
        $title                      = $data['action'].' '.$this->data['title'];
        $page_name                  = 'member-types/add-edit';        
        $data['row']                = [];
    ...
    {
        if(!$this->common_model->checkModuleFunctionAccess(12,60)){
            $data['action']             = 'Access Forbidden';
            $title                      = $data['action'].' '.$this->data['title'];
            $page_name                  = 'access-forbidden';        
            echo $this->layout_after_login($title,$page_name,$data);
